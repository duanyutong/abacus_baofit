...
def do_realisation(r, phase, model_name, overwrite=False, do_count=True,
    ...
    try:  # this helps catch exception in a worker thread of multiprocessing
        ...
        do_auto_smu = True
        do_fftcorr_pre_recon = True
        do_recon_std = True
        do_wp = True
        do_cross_count = True
        do_recon_ite = True
        if not overwrite and os.path.isfile(gt_path):
            ...
            if len(glob(temp+'-auto-fftcorr_N-pre-recon*.txt')) == 1:
                do_fftcorr_pre_recon = False
            if len(glob(temp+'-auto-fftcorr_N-post-recon-std*.txt')) == 1:
                do_recon_std = False
            if len(glob(temp+'-auto-wp-post-recon-std*.txt')) == 1:
                do_wp = False
        ...
        if do_count:  # require loading halocat
            ...
            subprocess.call(['python', './recon/read/read.py',
                             str(phase), str(seed)])
        if do_auto_smu:  # pre-recon auto-correlation with pair-counting
...
def convert_recon_correlation(model_name, phases):
    ...
    for i in range(3):
        ...
        for phase in phases:
            ...
            r = N[:, 1]
            xi_0 = N[:, 3] / R[:, 3] * np.square(b)
            xi_2 = N[:, 4] / R[:, 3] * np.square(b)
...
if __name__ == "__main__":
        ...
        for model_name in model_names:
            ...
            with closing(MyPool(processes=N_concurrent_reals,
                ...
                p.map(partial(do_realisation,
                              phase=phase, model_name=model_name,
                              do_count=True, do_auto_corr=True,
                              do_subcross_corr=True, overwrite=False),
                      range(N_reals))
            p.close()
            p.join()
    ...
    for model_name in model_names:
        coadd_phases(model_name, coadd_phases=range(16))
        do_cov(model_name, N_sub=N_sub, cov_phases=range(16))
        convert_recon_correlation(model_name, phases=range(16))
